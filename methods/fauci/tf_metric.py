...
def discrete_demographic_parity(protected: tf.Tensor, y_pred: tf.Tensor, weights_strategy: int = Strategy.EQUAL
                                ) -> tf.Tensor:
    """
    ...
    if weights_strategy == Strategy.EQUAL:
        ...
        return tf.reduce_sum(tf.abs(probabilities - absolute_probability) * number_of_samples) / total_number
    elif weights_strategy == Strategy.INVERSE_FREQUENCY:
        return tf.reduce_sum(
            tf.abs(probabilities - absolute_probability) * (total_number - number_of_samples)) / total_number
...
def discrete_equalized_odds(
        protected: tf.Tensor, y_true: tf.Tensor, y_pred: tf.Tensor, weights_strategy: int = Strategy.EQUAL
) -> tf.Tensor:
    """
...
def continuous_equalized_odds(
    ...
    differences_1 = tf.math.multiply_no_nan(differences_1, number_of_samples_a_1)
    total_samples = tf.reduce_sum(number_of_samples_a_0) + tf.reduce_sum(number_of_samples_a_1)
    return tf.math.divide_no_nan((tf.reduce_sum(differences_0) + tf.reduce_sum(differences_1)), total_samples)
